...
public class ServiceController {
    ...
    public ResponseEntity<Long> addServicesTariff(
            ...
            @RequestParam(value = "tariffId") Long tariffId, HttpServletRequest request
    ) {//return id
        String token = request.getHeader("Authorization");
        if (!authService.checkAuth(token)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
    ...
    public ResponseEntity<String> changeTariffById(
            @RequestParam(value = "operatorId") Long operatorId,
            @RequestBody Tariffs newValue, HttpServletRequest request
    ...
    public ResponseEntity<Long> addTariff(
            @RequestParam(value = "operatorId") Long operatorId,
            @RequestBody Tariffs value, HttpServletRequest request
    ) {
        String token = request.getHeader("Authorization");
        if (!authService.checkAuth(token)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
    ...
    public ResponseEntity<String> changeService(
            @RequestParam(value = "operatorId") Long operatorId,
            @RequestBody Services services, HttpServletRequest request
    ) {
        String token = request.getHeader("Authorization");
        if (!authService.checkAuth(token)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
